...
- **Workflows** view workflow rows now include a **play** button to dispatch a workflow on the repository default branch (latest commit) via the Gitea Actions `workflow_dispatch` API.
- New command: **Gitea Actions: Dispatch Workflow** (`giteaActions.dispatchWorkflow`), also available from the workflow row inline actions and context menu.

## [2.6.2] - 2026-06-13

### 🐛 Fixed

- Cancelled workflow runs no longer show multi-decade durations (e.g. **494801h**) when Gitea returns a zero `started_at` but valid `created_at`/`updated_at` timestamps; duration now skips epoch values and falls back the same way trigger time does.
- Workflow run timestamp mapping now uses `pickTimestamp` so a zero `started_at` no longer prevents falling back to `run_started_at`, and zero `completed_at` values no longer block valid alternate end timestamps.
